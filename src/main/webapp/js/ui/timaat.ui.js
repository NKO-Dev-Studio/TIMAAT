  ...
  TIMAAT.UI = {
    ...
    init: function () {
      ...
      $('#logInUserName').on('keyup', function (e) {
        if (e.keyCode == 13) jQuery('#loginSubmitButton').click();
      });
      ...
      $('#userLogList').on('inserted.bs.popover', function () {
        TIMAAT.Service.getUserLog(TIMAAT.Service.session.id, 12, function (log) {
          ...
          $(log).each(function (index, entry) {
            TIMAAT.Service.idCache.set(entry.userAccount.id, entry.userAccount.displayName);
          });
    ...
    sendNotification(type, dataID = 0, data = null) {
      if (!this.notificationSocket || this.notificationSocket.readyState == WebSocket.CLOSED) {
        ...
        this.notificationSocket.onopen = function () {
          ...
          TIMAAT.UI.notificationSocket.send(JSON.stringify({
            token: TIMAAT.Service.token,
            request: request,
            dataID: storedID
          }));
        }
      ...
      if (user.length > 0 && pass.length > 0) {
        ...
        setTimeout(function () {
          ...
          $.ajax({
            ...
            // Load enabled feature information
            const transcriptionSettings = await TIMAAT.SystemSettingsService.getTranscriptionSettings()
            if (transcriptionSettings.featureEnabled) {
              TIMAAT.UI.enabledFeatures.add("TRANSCRIPTION")
            }
    ...
    displayDataSetContent: function (form, data, type, mode = 'show') {
      ...
      switch (type) {
        ...
        case 'medium':
          ...
          const $mediumTranscriptionTabNavItem = $("#mediumTranscriptionTabNavItem")
          if (TIMAAT.UI.enabledFeatures.has("TRANSCRIPTION") && (subtype === "audio" || subtype === "video")) {
            $mediumTranscriptionTabNavItem.show();
          } else {
            $mediumTranscriptionTabNavItem.hide();

            if (form === "transcriptions") {
              TIMAAT.UI.displayComponent('medium', 'mediumTab', 'mediumDataTable', 'mediumMetadataTab', 'mediumFormMetadata');
              form = "mediumMetatdataTab"
            }
          }
